...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (args[0] != "embed" && args[0] != "color" && args[0] != "title" && args[0] != "footer" && args[0] != "channel" && args[0] != "image" && args[0] != "on" && args[0] != 'off' && args[0] != "text") return message.channel.send({ embeds: [{ color: "RED", title: "Ошибка ❌", description: `**${message.author}, укажите \`верный\` аргумент!**` }]}); let n;
        if (args[0] == "image") n = 1;
        if (args[0] == "text") n = 2;
        ...
        if (n == 2) {
            ...
            col1.on("collect", async (m) => {
                ...
                if (!m.content.split(' ')[0]) return message.channel.send({ embeds: [{ color: "RED", title: "Ошибка ❌", description: `**${message.author}, Вы не указали значение!**` }]});
                let a = m.content;
                if (a == "off") return guild.lv = null, message.channel.send({ embeds: [{ color: "GREEN", title: "Успешно ✅", description: `**${message.author}, Вы настроили необходимое значение!**` }]});
                if (a.length > 300 || a.length < 1) return message.channel.send({ embeds: [{ color: "RED", title: "Ошибка ❌", description: `**${message.author}, Укажите длину текста меньше 300 символов!**` }]});
                guild.lv = a;
                guild.save()
                return message.channel.send({ embeds: [{ color: "GREEN", title: "Успешно ✅", description: `**${message.author}, Вы настроили необходимое значение!**` }]});
            })
        ...
        if (n == 3) {
            ...
            if (!m.content) return message.channel.send({ embeds: [{ color: "RED", title: "Ошибка ❌", description: `**${message.author}, вы не указали значения!**` }]});
            if (!m.content.split(' ')[0]) return message.channel.send({ embeds: [{ color: "RED", title: "Ошибка ❌", description: `**${message.author}, Вы не указали значение!**` }]});
            if (m.content.split(' ')[0] == "off") return guild.leave = null, guild.save(),message.reply("успешно!");
    ...
    if (n == 4) {
        ...
        col1.on("collect", async (m) => {
            ...
            if (!m.content.split(' ')[0]) return message.channel.send({ embeds: [{ color: "RED", title: "Ошибка ❌", description: `**${message.author}, Вы не указали значение!**` }]});
            let a = m.content;
            if (a == "off") return guild.lft = null, message.channel.send({ embeds: [{ color: "GREEN", title: "Успешно ✅", description: `**${message.author}, Вы настроили необходимое значение!**` }]});
        ...
        if (n == 5) {
            ...
            col1.on("collect", async (m) => {
                ...
                if (!m.content.split(' ')[0]) return message.channel.send({ embeds: [{ color: "RED", title: "Ошибка ❌", description: `**${message.author}, Вы не указали значение!**` }]});
                let a = m.content;
                if (a == "off") return guild.lt = null, message.channel.send({ embeds: [{ color: "GREEN", title: "Успешно ✅", description: `**${message.author}, Вы настроили необходимое значение!**` }]});
                if (a.length > 15 || a.length < 1) return message.channel.send({ embeds: [{ color: "RED", title: "Ошибка ❌", description: `**${message.author}, Укажите длину текста меньше 25 символов!**` }]});
                guild.lt = a;
                guild.save()
                return message.channel.send({ embeds: [{ color: "GREEN", title: "Успешно ✅", description: `**${message.author}, Вы настроили необходимое значение!**` }]});
        ...
        if (n == 6) {
            ...
            col1.on("collect", async (m) => { 
                ...
                let c;
               c = m.content;
               if (c == "off") guild.lcolor = null,guild.save(),message.reply("успешно!");
